...
@st.cache_resource
def load_models():
    # Model A is loaded from the Hugging Face Hub
    ner_pipeline = pipeline("ner", model="eltgnd/ac-health-ner", 
                            tokenizer=AutoTokenizer.from_pretrained("microsoft/BiomedNLP-BiomedBERT-base-uncased-abstract-fulltext"))
    
    # Model B is now loaded from the Hugging Face Hub
    model_b = AutoModelForSequenceClassification.from_pretrained(
        'eltgnd/ac-health-classifier',
...
def process_and_update_names(df: pd.DataFrame, candidate_indices: List, model_b_classifier_pipeline) -> pd.DataFrame:
    ...
    if not mask_base.any():
        st.write("No rows need processing. Skipping.")
        return df
    ...
    if not texts_to_process:
        st.write("No valid text found in rows to update. Skipping.")
        return df

    st.write(f"Processing {len(texts_to_process)} unique names in a single batch...")
    ...
    if mask_single.any():
        st.write("Applying logic to single-name rows...")
    ...
    if mask_dual.any():
        st.write("Applying logic to dual-name rows...")
...
def process_drug_data(
    ...
    st.write('Generating entity predictions with Model A')
    ...
    st.write('Creating columns from predicted entities')
    ...
    st.write('Flagging generic and brand names that are in the English dictionary')
    ...
    st.write('Applying model B refinement logic')
    ...
    st.write('Processing non-candidate strings using model B...')
    ...
    for idx, row in df.iterrows():
        ...
        if isinstance(flavor, str) and isinstance(input_string, str):
            start = input_string.lower().find(flavor)
            df.at[idx, 'flavor'] = input_string[start:start+len(flavor)]

    # Final cleanup of unnecessary columns
    df.drop(columns=['cleaned_ner', 'processed_string'], inplace=True, errors='ignore')

    reordered = ['input_string', 
                 'generic_name',
                 'brand_name', 
                 'dosage_strength',
                'dosage_pack', 'dosage_form', 'volume',
                'total_individual_units', 'flavor',
                'generic_name_checked',
                'brand_name_checked',
                'score_orig_generic_name', 'score_orig_brand_name',
                'score_orig_dosage_strength', 'score_orig_dosage_form',
                'score_orig_dosage_pack', 'score_orig_volume',
                'score_orig_total_individual_units', 'score_orig_flavor',
                'score_new_generic_name', 'score_new_brand_name',
                'generic_name_in_english_dictionary',
                'brand_name_in_english_dictionary', ]
...
if input_method == "Enter Text":
    ...
    text_input = st.text_area(
        ...
        "Paracetamol 500mg Tablet (Biogesic)\nBIOGESIC Paracetamol (Tablet) 500 mg\nParacetamol Biogesic PARACETAMOL - 500MG [tablet]",
    ...
    if uploaded_file is not None:
        try:
            ...
            with st.status(f"Analyzing {len(input_data)} rows..."):
            ...
            excel_output = convert_df_to_excel(df_output)

            st.download_button(
                label="📥 Download results as Excel",
                data=excel_output,
                file_name='processed_drug_entities.xlsx',
